// ============================================================
// Agent 33 - Publish Agent
// Phase: DISTRIBUTE | Model: gemini-2.5-flash | Depends on: account-manager
// ============================================================
// Handles publishing to articles table and social platforms
// ============================================================
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (runId) {
      for (const dep of ["account-manager"]) {
    ...
    const prompt = `You are a specialized AI agent (Publish Agent) for Pakistani digital media platform LADtoday.
Topic: "${topic}"
...
Generate the final publish payload for each destination in the routing plan. For the articles table: slug, meta, SEO fields. For Facebook: caption + link. For Twitter: thread. For email: newsletter trigger. Include publish confirmation checklist.`;
