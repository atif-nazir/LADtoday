// ============================================================
// Agent 25 - Video Script Agent
// Phase: MULTIMEDIA | Model: gemini-2.5-flash | Depends on: rewrite,creative
// ============================================================
// Generates YouTube/Facebook video scripts with B-roll suggestions and chapters
// ============================================================
...
const AGENT_KEY = "video-script";
const AGENT_NAME = "Video Script Agent";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (runId) {
      for (const dep of ["rewrite","creative"]) {
    ...
    const prompt = `You are a specialized AI agent (Video Script Agent) for Pakistani digital media platform LADtoday.
Topic: "${topic}"
...
Create a video script: Hook (0:00-0:30) with most surprising fact, Problem setup (0:30-1:30), Main content (1:30-6:00) with B-roll suggestion and text overlay every 45-60s, Resolution (6:00-7:00), CTA (7:00-7:30). Include chapter_markers for YouTube and thumbnail brief.`;
