// ============================================================
// Agent 35 - Hashtag Strategy Agent
// Phase: DISTRIBUTE | Model: gemini-2.5-flash | Depends on: seo,short-form,thread
// ============================================================
// Generates platform-specific research-backed hashtag sets
// ============================================================
...
const AGENT_KEY = "hashtag-strategy";
const AGENT_NAME = "Hashtag Strategy Agent";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (runId) {
      for (const dep of ["seo","short-form","thread"]) {
    ...
    const prompt = `You are a specialized AI agent (Hashtag Strategy Agent) for Pakistani digital media platform LADtoday.
Topic: "${topic}"
...
Generate strategic hashtag sets. Instagram (15): 2 mega >1M, 5 large 100K-1M, 5 medium 10K-100K, 3 small <10K. Twitter (3): 1 trending + 1 evergreen + 1 brand. LinkedIn (5): professional/industry. TikTok (6): 3 trending + 2 niche. Flag tags to AVOID (shadowban risk, toxic).`;
