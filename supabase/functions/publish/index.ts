...
// ─── WORDPRESS or LOCAL ARTICLES TABLE ──────────────────────────────────────

async function publishToSite(article: any, runId: string) {
  // Prefer external WordPress if configured, otherwise insert into local articles table.
  if (WORDPRESS_URL && WORDPRESS_JWT) {
    try {
      const response = await fetch(`${WORDPRESS_URL}/wp-json/wp/v2/posts`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${WORDPRESS_JWT}`, "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!response.ok) {
        ...
        return { published: false, target: "wordpress", reason: errorText.slice(0, 200) };
      }
      const data = await response.json();
      return { published: true, target: "wordpress", post_id: data.id, url: data.link };
    } catch (err) {
      // fall through to local
      console.warn("[publish] WordPress failed, falling back to local articles table:", err);
    }
  }

  // ── Local LADtoday articles table ──
  try {
    ...
    const slug = (article.url_slug || article.headline || "article")
      .toString().toLowerCase().replace(/[^a-z0-9]+/g, "-").slice(0, 80) + "-" + runId.slice(0, 6);

    const html = article.article_html || `<p>${article.body || article.headline}</p>`;
    // Build a single-section structure compatible with src/data/articles.ts consumers
    const sections = [{ heading: article.headline || "", paragraphs: [html] }];

    const insertRow: any = {
      title: article.headline || "Untitled",
      subtitle: article.meta_description || null,
      slug,
      category_id: cat?.id,
      date: new Date().toISOString().slice(0, 10),
      read_time: `${Math.max(1, Math.round((article.word_count || 600) / 220))} min read`,
      image: article.hero_image_url || "https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=1200",
      author_name: "LADtoday AI Desk",
      author_bio: "Generated by the LADtoday agent pipeline.",
      introduction: article.meta_description || article.headline,
      sections,
      conclusion: article.body ? article.body.slice(-600) : null,
      tags: article.tags || [],
      published: true,
      ai_title: article.headline,
      ai_content: html,
      ai_summary: article.meta_description,
      ai_generated_at: new Date().toISOString(),
      fb_caption: article.social_snippets?.facebook || null,
    };
    ...
    if (error) return { published: false, target: "local", reason: error.message };

    const url = `/article/${data.slug}`;
    return { published: true, target: "local", post_id: data.id, url, slug: data.slug };
  } catch (err) {
