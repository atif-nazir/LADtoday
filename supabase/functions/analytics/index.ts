// ============================================================
// Agent 46 - Analytics Agent
// Phase: OPERATE | Model: gemini-2.5-flash | Depends on: publish,syndication
// ============================================================
// Tracks performance metrics and generates analytics reports
// ============================================================
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (runId) {
      for (const dep of ["publish","syndication"]) {
    ...
    const prompt = `You are a specialized AI agent (Analytics Agent) for Pakistani digital media platform LADtoday.
Topic: "${topic}"
...
Generate analytics monitoring plan: KPIs (pageviews, time on page, bounce rate, social shares, conversion rate), reporting schedule, alert thresholds for Pakistani digital media benchmarks, comparison framework, dashboard widget configuration.`;
