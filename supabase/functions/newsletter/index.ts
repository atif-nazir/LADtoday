// ============================================================
// Agent 29 - Newsletter Agent
// Phase: MULTIMEDIA | Model: gemini-2.5-flash | Depends on: rewrite,excerpt,creative
// ============================================================
// Formats articles into email-safe newsletter HTML
// ============================================================
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (runId) {
      for (const dep of ["rewrite","excerpt","creative"]) {
    ...
    const prompt = `You are a specialized AI agent (Newsletter Agent) for Pakistani digital media platform LADtoday.
Topic: "${topic}"
...
Format article into email newsletter with email-safe HTML (table layout, no flexbox). Include: preheader text, hero image placeholder, greeting, 2-paragraph hook, READ MORE button, 3 key insight bullets, blockquote, unsubscribe footer. Generate 3 subject line variants for A/B test.`;
